...
async function maybeUpdateResearcherProfile(userId, newSearchCount) {
  ...
  const { data: recent } = await supabaseAdmin.from('search_history').select('target_searched, goal_input').eq('user_id', userId).order('created_at', { ascending: false }).limit(5);
  if (!recent?.length) return;
  const historyText = recent.map((s, i) => `${i + 1}. Target(s): ${s.target_searched} | Goal: ${s.goal_input || 'n/a'}`).join('\n');
  ...
  try {
    const res = await fetchWithRetry('https://api.groq.com/openai/v1/chat/completions', { method: 'POST', headers: { 'Content-Type':'application/json', 'Authorization':`Bearer ${process.env.GROQ_API_KEY}` }, body: JSON.stringify({ model:'openai/gpt-oss-120b', messages:[{role:'system',content:system},{role:'user',content:historyText}] }) }, 1, 6000);
    const data = await res.json();
    const synth = data?.choices?.[0]?.message?.content?.trim();
    if (synth) await supabaseAdmin.from('profiles').update({ researcher_profile: synth }).eq('id', userId);
  } catch (e) { console.error('Profile synth error:', e.message); }
}
...
function extractJSON(str) {
  ...
  try { return JSON.parse(json); } catch (e) {
    ...
    let fixed = json;
    while (stack.length) { const opener = stack.pop(); fixed += opener==='['?']':'}'; }
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (error && error.code === 'PGRST116') {
      await supabaseAdmin.from('profiles').insert({ id:user.id, email:user.email, tier:'Free', assays_used_this_month:0, usage_period:currentPeriod(), search_count:0 });
      const { data: newP } = await supabaseAdmin.from('profiles').select('*').eq('id',user.id).single();
      profile = newP;
    } else if (error||!data) throw error||new Error('Profile missing');
    else profile = data;
    console.log(`[${rid}] Profile – tier:${profile.tier}, used:${profile.assays_used_this_month}`);
  } catch(e) { console.error(`[${rid}] Profile error:`,e); return res.status(500).json({ error:'Profile service error.' }); }
  ...
  if (!isSuper) {
    ...
    if (used >= limit) return res.status(403).json({ error:`Limit reached (${profile.tier}: ${limit}). Upgrade needed.` });
  }
  ...
  try {
    ...
    try {
      const enhRes = await fetchWithRetry('https://api.groq.com/openai/v1/chat/completions', { method:'POST', headers:{'Content-Type':'application/json','Authorization':`Bearer ${process.env.GROQ_API_KEY}`}, body:JSON.stringify({ model:'openai/gpt-oss-120b', messages:[{role:'system',content:enhSystem},{role:'user',content:`Targets: ${targetsHeading}\nRaw Goal: ${goal||'General info'}`}] }) },2,6000);
      ...
      for (const [t,q] of Object.entries(rawQueries)) {
        if (Array.isArray(q)) optimizedQueries[t] = q.filter(s=>typeof s==='string'&&s.trim().length);
        else if (typeof q==='string'&&q.trim()) optimizedQueries[t] = [q];
    ...
    for (const target of targetsArray) {
      ...
      for (let qi=0; qi<queries.length; qi++) {
        ...
        try {
          ...
          if (papers.length) {
            const mapped = papers.map(p=>({ title:p.title||'Untitled', url:p.url||(p.paperId?`https://www.semanticscholar.org/paper/${p.paperId}`:''), year:p.year||'Unknown', abstract:p.abstract?.substring(0,400)+'...'||'', associatedTarget:target })).filter(p=>p.url);
            targetPapers.push(...mapped);
          }
          if (targetPapers.length>=8) break;
        } catch(err) { console.error(`[${rid}] S2 error for "${query}":`,err.message); }
      }
      if (targetPapers.length===0) { fallbackTriggered=true; console.log(`[${rid}] All queries failed for "${target}", marking fallback.`); }
    ...
    if (allPapers.length===0) {
      ...
      try {
        const lastRes = await fetchWithRetry('https://api.groq.com/openai/v1/chat/completions', { method:'POST', headers:{'Content-Type':'application/json','Authorization':`Bearer ${process.env.GROQ_API_KEY}`}, body:JSON.stringify({ model:'openai/gpt-oss-120b', messages:[{role:'system',content:'You are a search expert. Given a biomedical goal and target, output a single, extremely effective search query string (max 15 words) that would find relevant scientific literature. Return ONLY the query string, no JSON, no extra text.'}, {role:'user',content:`Targets: ${targetsHeading}\nGoal: ${goal||'General info'}`}] }) },1,5000);
        ...
        if (lastQuery&&lastQuery.length>3) {
          ...
          const s2Data = await s2Res.json();
          const papers = s2Data.data||[];
          const mapped = papers.map(p=>({ title:p.title||'Untitled', url:p.url||(p.paperId?`https://www.semanticscholar.org/paper/${p.paperId}`:''), year:p.year||'Unknown', abstract:p.abstract?.substring(0,400)+'...'||'', associatedTarget:targetsHeading })).filter(p=>p.url);
          allPapers.push(...mapped);
        }
      } catch(e) { console.warn(`[${rid}] Last-ditch failed:`,e.message); }
    }
...
Proceed with your reasoning pipeline and return the JSON.`;
    ...
    let finalJson;
    try { finalJson = extractJSON(rawText); }
    catch(e) {
...
Return ONLY the revised JSON in the same structure.`;
    ...
    await supabaseAdmin.from('profiles').update({ assays_used_this_month:usedNow, usage_period:period, search_count:newCount }).eq('id',user.id);
    await supabaseAdmin.from('search_history').insert([{ user_id:user.id, target_searched:targetsHeading, goal_input:goal }]);
